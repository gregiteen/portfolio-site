...
const IMAP_USER = process.env.IMAP_USER || process.env.MAIL_USER;
...
export async function getImapClient() {
  ...
  if (!IMAP_USER || !IMAP_PASS) throw new Error('IMAP credentials not configured');
...
export async function startImapPoller(smtpTransport) {
  ...
  c.on('exists', async (data) => {
    ...
    try {
      ...
      if (!fromAddr || fromAddr.includes('noreply') || fromAddr.includes('daemon') || fromAddr === IMAP_USER) {
        return; // Avoid loop
      }
      ...
      const mailOptions = {
        from: process.env.MAIL_FROM || IMAP_USER,
        to: fromAddr,
