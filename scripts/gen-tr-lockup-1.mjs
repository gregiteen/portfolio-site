...
async function run() {
  const trBase = "A professional tech startup logo lockup for 'TOTAL RECALL'. Layout: A highly creative, small distinct symbol on the far left, followed immediately on the right by the exact text 'TOTAL RECALL' written in a cool, modern, sleek font. Standard corporate lockup on a pure white background. Flat vector graphic style. NO SERVICE MARKS. NO EXTRA TEXT. NO BRAINS.";
  await generateImageParts([{text: `${trBase} Variation 1: Symbol is an elegant, abstract golden hourglass made of geometric light beams.`}], "assets/logos/tr_lockup_1.jpg", "TR 1");
  console.log("Done.");
}
