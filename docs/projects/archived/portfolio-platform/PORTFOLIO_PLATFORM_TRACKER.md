...
> `docs/projects/in-progress/PORTFOLIO_VISITOR_FUNNEL_RECOVERY/`. Historical
> verification records are preserved below; unchecked work was carried into
> the consolidated tracker as current verification or implementation tasks.
...
| **Proposal PDF + e-sign** | ✅ (2026-07-09) | ✅ real end-to-end API test | Phase 4 (4.1-4.3 done; 4.4 open) |
...
## ⏳ Phase 1: Verify the Claimed-Done Platform (old P0–P5 had zero tests) — 1.2/1.3 release-gate verification reopened 2026-07-09; prior live checks remain historical evidence only
...
- [x] 1.2a [portfolio] **release-gate E2E re-verification (2026-07-10)**: fresh production generation `editorial-brutalist-systems-studio-for-a` built 5/5 CSS sections + 10/10 layouts, repaired structural findings, then had `gemini-3.1-pro-preview` review the complete CSS + all actual layout bodies (7/10 → targeted repair → 10/10) before the skin document/static route existed. A separate Pro visual-asset review approved it; the route then returned 200. Rejected attempts left `latestUrl:null` and no public route.
- [x] 1.3a [portfolio] **serialized build + post-generation review re-verification (2026-07-10)**: child generation deferred the build to `serve.mjs`; the one server owner built the skin after the pre-release gate. The legacy post-public auto-improver is now disabled so a lower-assurance pass cannot replace an approved skin after visitors see it.
...
- [x] 4.1 [portfolio] PDF renderer chosen (pdfkit — see rationale below) and rendering finalized proposals cleanly; letterhead PDF verified visually (single page, no stray blank page after fixing a pdfkit auto-pagination bug — see rationale)
- [x] 4.2 [portfolio] PDF attached on BOTH send paths — `sendProposalToClient()` (serve.mjs:580) is the single shared function called from both `/api/proposal-reply` "send it" (serve.mjs:1424) and the admin/TR decision endpoint `/api/admin/proposals/:id/decision` (serve.mjs:1578); cover-note email body includes the signing link block when available
- [x] 4.3 [portfolio] **E-sign: Documenso chosen over DocuSeal — verified live 2026-07-09.** DocuSeal was deployed first (container, nginx, DNS, TLS, admin account) and rejected AFTER discovering its self-hosted API is paywalled: `POST /submissions/pdf` → `404 {"message":"This feature is available in Pro Edition"}`, despite the project being AGPL/"open source." Documenso deployed instead (`/opt/documenso/`, compose.yml + MinIO for S3-compatible storage, `sign.gregiteen.xyz`). **Real end-to-end API test performed** (not just docs-reading): created a document via `POST /api/v1/documents`, uploaded a real PDF to the presigned S3 URL, added a signature field, sent it, and loaded the resulting `/sign/<token>` page — rendered the actual PDF with a working signature UI. Confirmed the **client-facing signing page has effectively zero visible Documenso branding when logged out** (no navbar, no logo/wordmark in the rendered HTML — verified via a cookie-less `curl` fetch, only 2 inert "Documenso" mentions: the `<title>` tag and a CSS class hook). Applied full custom branding anyway (logo, brand website, brand colors matching the site's `--ink/--bone/--accent/--line` tokens) via `/settings/branding`. **Signing link + status wired**: `scripts/lib/documenso.mjs` (replaces the deleted `docuseal.mjs`) returns `{signingUrl, submissionId}`, stored as `thread.signingUrl` and included in the client email. `DOCUMENSO_BASE_URL`/`DOCUMENSO_API_KEY` added to `.env.example` and the droplet's `/opt/portfolio-site/.env` (not yet deployed/restarted — pending Greg's go-ahead per the no-auto-deploy rule).
...
| 2026-07-08k | 1.2/atelier | (continues 08j) `compile-theme.mjs` generation was still a serial monolith: Pass 2a emitted the ENTIRE CSS design system + shell in one ~55s call, and the planning/foundation calls **flakily exploded** to a ~57K-token output → `MAX_TOKENS` truncation → **181-188s + slow antigravity fallback** (full runs measured at 249s/291s). Violated the max-parallelization intent — one response should never be that long. | Root cause: (a) one monolithic CSS call; (b) gemini-3.5-flash is a *thinking* model whose thinking+output share `maxOutputTokens`, and open-ended prompts (plan self-critique, foundation) flakily ran the **output** away to 65K. A first attempt to fix via a global `maxOutputTokens=32768` made it **worse** (capped output *below* the runaway, forcing truncation every time). | **Decomposed generation into director + full parallel fan-out** (verified 2026-07-08): **(1)** merged plan + plan-review into one self-critique pass; **(2)** replaced the monolithic Pass 2a with a **director → StyleSpec** (small; enumerates the shared token + CSS-class vocabulary) then **one `Promise.all` of 5 CSS-section specialists + 10 layout specialists** — no single long response, all cohere to the shared vocabulary; **(3)** **few-shot exemplar** on every layout specialist; **(4)** **bounded thinking** (`thinkingConfig.thinkingBudget=8192`) + **per-call `maxOutputTokens`** (planning 16384, director 24576, specialists 65536) so a flaky runaway truncates in seconds, not at 65K; **(5)** **`MAX_TOKENS` guard** in `geminiApiCall` (fail loud instead of returning silently-truncated unterminated JSON); **(6)** wired the **post-generation auto-improve trigger** into `serve.mjs` (captures the slug from compile-theme's `→ designs/<slug>` line, spawns `improve-theme <slug>` async → scores → parallel-regenerates weak slots → hot-swaps → rebuilds). **Verified**: 3 consecutive full builds (distinct briefs) → **51s / 60s / 54s**, every call `finish=STOP` (zero truncations, zero fallbacks), 5/5 CSS + 10/10 layouts each; coherence spot-check 13/17 layout classes defined in the parallel CSS (gaps are utility/token names the improve pass fills); `npm test` **14/14**. Down from the 97s serial baseline and the 249-291s flaky failures. Removed now-dead `FULL_PAYLOAD_SCHEMA`/`LAYOUTS_ONLY_SCHEMA`/`LAYOUTS_OBJECT_SCHEMA`. |
...
| 2026-07-09 | 1.2/1.3 | The async post-generation improve job was assumed to be a pre-release whole-page quality gate. | Live code/log review showed the initial skin was saved and served before auto-improve started; its scorer received only the first 3,000 CSS chars plus layout names, not the actual HTML. Lenient validation could drop required layouts, and direct generator/improver builds raced vault watcher builds (`ENOTEMPTY` in `dist/site`). A local `ADMIN_API_TOKEN` had also drifted from the droplet token. | Added a strict pre-write release gate: every layout, CSS structure, placeholders, visible-copy rule, and hero asset are validated; a model audits the complete CSS + all layout bodies and repairs only named failing slots before publish. Generator/improver children now defer builds to one server owner; post-improve scoring receives the full source. The local token was securely synchronized from the host; the admin API deleted `nordic-wood` and `midnight-teal-art-deco-jazz-lounge-with-` and both public URLs returned 404. Fresh live generation proof remains 1.2a/1.3a. |
| 2026-07-09 | 4.3 | DocuSeal (docuseal.com), AGPL-licensed and marketed as "open source," would provide a genuinely usable self-hosted signing API once deployed. | After **fully deploying it** (Docker container, nginx vhost, DNS A record, Let's Encrypt TLS, admin account created) the self-hosted API turned out to be **entirely paywalled**: `POST /submissions/pdf` → `404 {"message":"This feature is available in Pro Edition"}` — same result for every API/embedding endpoint tested. "Open source" only covered the source-availability of the core app; API access requires a $20/mo Pro license even self-hosted. | Deployment torn down; switched to **Documenso** (also AGPL) instead, this time verifying the API is genuinely free **before** declaring success: real end-to-end test — create document via API → upload real PDF to presigned URL → add signature field → send → load the resulting signing page and confirm it renders the actual PDF with a working signature UI. Documenso's self-hosted API additionally required **S3-compatible storage** (local filesystem storage only supports the web-UI upload flow) — self-hosted MinIO added rather than paying for cloud object storage, first bound to the droplet's private VPC IP (works server-to-server) then moved to a real HTTPS endpoint (`s3.gregiteen.xyz`, new DNS record + nginx + certbot) once discovered that **browser-based** uploads (e.g. the branding-logo upload in Documenso's own settings UI) fail against a private-IP/mixed-content endpoint even though server-to-server calls from `serve.mjs` don't care. Lesson saved to Total Recall as a standing global rule: always verify a self-hosted "open source" tool's *specific* API/feature actually works via a live test before recommending or building on it — source-available ≠ API-free. |
...
**Chosen: `pdfkit`** (added to `package.json` deps). Rationale: pure-JS, no
headless-browser/native-binary dependency (unlike puppeteer-based PDF
generation), small footprint, sufficient for a text-heavy letterhead
document. `scripts/lib/letterhead.mjs` renders Greg's wordmark ("greg.iteen"
text, not an image logo — the only source PNG has an opaque black
background that would print as a black box on white letterhead), contact
line, title block, minimal-markdown body, and a per-page footer.

**Bug found + fixed during implementation**: writing footer text near the
56pt bottom margin made pdfkit think content overflowed and silently start
a second, blank page for the footer alone. Fixed by zeroing
`doc.page.margins.bottom` during the footer write only, restoring it after.
Verified visually (before: 2 pages with a stray blank page; after: clean
single page).
...
| `compile-theme.mjs` | AI theme generation (director StyleSpec → parallel fan-out of 5 CSS-section + 10 layout specialists; ~55s, no serial monolith — see 08k) |
| `improve-theme.mjs` | Daily + post-generation improvement (parallel specialist fan-out, hot-swap) with model rotation |
