...
- [ ] 1.2a [portfolio] **release-gate E2E re-verification (2026-07-09)**: a fresh generation must show the full CSS + all 10 HTML layouts are deterministically validated, then included in the LLM release review **before** the skin source/static page is published; rejected output must leave no new public route. Unit/SSSS/build checks are green locally; live generation proof remains required.
- [ ] 1.3a [portfolio] **serialized build + post-generation review re-verification (2026-07-09)**: prove the source-only generator/improver children defer to `serve.mjs`'s single build owner (no `ENOTEMPTY` race), and that the post-generation scorer receives every layout body rather than only layout names.
